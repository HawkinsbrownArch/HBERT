...
namespace HBERT
{
    internal class TreeChart
    {
        ...
        {
            ElementId invisibleLinesId = FilledRegionHelper.GetInvisibleLineStyleId(doc);
            ...
            foreach (Dictionary<string, object> rectangle in paddedRects)
            {
                ...
                List<CurveLoop> rectangleBoundaries = new FilledRegionHelper().GenerateCurveLoop(rectangle, width, height, smallCurveTolerance, out origin);
                if(rectangleBoundaries != null)
                {
                    ElementId typeId = FilledRegionHelper.GetTypeId(doc, rectangle["material"].ToString(), filledRegionTypeDictionary);
                    ...
                    annotation.TextPointSize.Add(Annotation.FindBestTextPointSize(height, width, convertToFt));
